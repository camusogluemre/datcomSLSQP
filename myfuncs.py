_DATCOM_EXE_PATH = None
_DATCOM_DAT_PATH = None

def set_datcom_exe_path(path):
    global _DATCOM_EXE_PATH
    _DATCOM_EXE_PATH = path

def set_datcom_dat_path(path):
    global _DATCOM_DAT_PATH
    _DATCOM_DAT_PATH = path

def _get_datcom_exe_path():
    import os
    if _DATCOM_EXE_PATH and os.path.exists(_DATCOM_EXE_PATH):
        return _DATCOM_EXE_PATH
    local = os.path.join(os.path.dirname(os.path.abspath(__file__)), "digital_DATCOM.exe")
    return local

def _get_datcom_work_dir():
    import os
    return os.path.dirname(_get_datcom_exe_path())

def _get_work_dat_path(default_name="for005.dat"):
    import os
    return os.path.join(_get_datcom_work_dir(), default_name)

def _get_out_path():
    import os
    return os.path.join(_get_datcom_work_dir(), "datcom.out")

def _run_datcom_and_wait(timeout_s=20.0, poll_s=0.25):
    import os, time, subprocess
    exe = _get_datcom_exe_path()
    work_dir = _get_datcom_work_dir()
    ...
    subprocess.Popen(exe, startupinfo=startupinfo, cwd=work_dir)
    ...
    raise FileNotFoundError(f"datcom.out was not created in: {work_dir}")

def findDatcomIdx(fileName):
    test005r = open(fileName, 'r')
...
def wing_optimization(inp, iTotalHeader, iTotalc, iTotalr, ZV, test005r_words, test005r_lines, weight, opt_scores):
    ...
    #SSPNE girdisi SSPN'den kuyruğun bulunduğu konumdaki gövdenin yarıçapından çıkarılarak elde edilir. 
        ...
        with open(_get_work_dat_path(), 'w') as test005:
          test005.writelines(test005r_lines)

        test005.close()

        _run_datcom_and_wait()

        with open(_get_out_path(), 'r') as results:
